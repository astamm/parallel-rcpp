...
The fact that the `dist` function takes a list of matrices as input can be handled by [{Rcpp}](https://rcpp.org/). However, there is no threadsage wrapper for lists due to the fact that they can store objects of different types and thus cannot be serialized.
...
bm <- bench::mark(
  ...
  openmp5 = dist_cpp_omp(dat, dimension = 3L, ncores = 4L),
  rcppparallel1 = dist_rcppparallel(dat, dimension = 3L, ncores = 1L),
  rcppparallel5 = dist_rcppparallel(dat, dimension = 3L, ncores = 4L)
)
```
...
- Using nested parallelization does not help (`nested` entry).
