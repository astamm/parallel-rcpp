...
format:
  revealjs:
    footer: "High-Performance Computing with R - Fréjus - <aymeric.stamm@cnrs.fr>"
    slide-number: true
...
- [RcppThread](https://tnagler.github.io/RcppThread/) [@nagler2021r]
...
on macOS as OpenMP flags are handled in `~/.R/Makevars`**. Uncomment (remove the
first `//`) to use on Windows and Linux.
...
std::vector<unsigned int> parallelfor_example(unsigned int n)
{
  // Index-based
  std::vector<unsigned int> x(n);
  RcppThread::parallelFor(0, x.size(), [&x] (unsigned int i) {
    x[i] = i;
  });
  return x;
}
...
std::vector<unsigned int> parallelforeach_example(unsigned int n)
{
  ...
  RcppThread::parallelForEach(x, [] (unsigned int& xx) {
    xx *= 2;
  });
